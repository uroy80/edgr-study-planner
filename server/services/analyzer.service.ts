...
function normalizeUnits(input: unknown): RUnit[] {
  const arr = Array.isArray((input as { units?: unknown })?.units)
  ...
  for (const u of arr) {
    ...
    if (!title) continue;
    const topicsRaw = Array.isArray(uo.topics) ? uo.topics : [];
    const topics: RTopic[] = [];
    for (const t of topicsRaw) {
      ...
      if (!ttitle) continue;
      const subs = Array.isArray(to.subtopics)
        ? (to.subtopics as unknown[]).map((x) => String(x).trim()).filter(Boolean).slice(0, 8)
        : [];
      topics.push({ title: ttitle.slice(0, 200), subtopics: subs });
    }
    units.push({
      title: title.slice(0, 200),
      summary: String(uo.summary ?? '').trim().slice(0, 500),
...
Return the roadmap JSON now.`;
    ...
    const raw = await chat(
      ...
      { json: true, temperature: 0.2, timeoutMs: 240_000 },
    );
